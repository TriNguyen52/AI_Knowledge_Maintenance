...
Tabs:
    1. Dashboard        — health score, trend, actions, approval workflow, problems, signals, history
    2. Artifact Explorer — inline content view with deduplicated signal annotations
    3. Memory           — A/B test, EMA scores, pause/resume
    4. Technical        — MCP tools, cross-check diagnostics, AWS event proof
...
# ---------------------------------------------------------------------------
# Result rendering helpers — human-readable summaries from Lambda responses
# ---------------------------------------------------------------------------

def _render_result_summary(result: dict[str, Any]) -> None:
    """Render a Lambda result dict as human-readable Streamlit elements.

    Shows key metrics prominently; raw data in an expandable section.
    """
    # Determine result type by available fields
    if "score_before" in result and "score_after" in result and "verification_outcome" in result:
        # Verification / remediation result
        ...
        if signals_new:
            st.warning(f"{signals_new} new signals introduced during remediation")
        if modified_uris:
            shown = ", ".join(str(u) for u in modified_uris[:5])
            extra = f" +{len(modified_uris) - 5} more" if len(modified_uris) > 5 else ""
            st.caption(f"Modified {len(modified_uris)} artifacts: {shown}{extra}")

    elif "score" in result and "signal_count" in result:
        # Assessment result
        score = result.get("score", 0)
        signal_count = result.get("signal_count", 0)
        problems = result.get("problems_discovered", 0)
            ...
            for key, val in result.items():
                if isinstance(val, (str, int, float, bool)):
                    ...
                    for item in val[:5]:
                        st.markdown(f"  - {item}")
                    if len(val) > 5:
                        st.caption(f"... and {len(val) - 5} more")
                else:
                    st.markdown(f"**{key}**: {val}")
        else:
            st.write(result)
...
def _render_proposal_review(proposal: dict[str, Any], tab_key: str) -> tuple[int, bool, str] | None:
    """Render proposal selection UI with impact summary.

    Following Linear-style restraint: clean sections, no cards, clear hierarchy.
    User selects a proposal, sees its impact, then approves or rejects.

    Returns (selected_proposal_idx, approved, reason) or None if no proposals.
    """
    proposals = proposal.get("proposals", [])
    if not proposals:
        st.caption("No proposals generated.")
        return None
    ...
    st.markdown("**Select a Proposal**")

    # Build proposal labels with impact summary
    proposal_labels = []
    for i, p in enumerate(proposals):
        strategy = p.get("strategy", "unknown")
        confidence = p.get("confidence", 0)
        steps = p.get("modification_steps", [])
        n_artifacts = len(set(s.get("artifact_uri", "") for s in steps))
        n_steps = len(steps)
        label = f"Proposal {i + 1}: {strategy} — {n_steps} steps, {n_artifacts} artifacts, {confidence:.0%} confidence"
        proposal_labels.append(label)

    selected_idx = st.radio(
        "Select proposal",
        range(len(proposal_labels)),
        format_func=lambda i: proposal_labels[i],
        key=f"proposal_select_{tab_key}",
    )

    selected = proposals[selected_idx]
    steps = selected.get("modification_steps", [])

    # Impact summary — inline metrics, not boxed cards
    col_i1, col_i2, col_i3 = st.columns(3)
    with col_i1:
        st.metric("Modification Steps", len(steps))
    with col_i2:
        unique_artifacts = set(s.get("artifact_uri", "") for s in steps)
        st.metric("Artifacts Affected", len(unique_artifacts))
    with col_i3:
        st.metric("Confidence", f"{selected.get('confidence', 0):.0%}")
    ...
    # Reasoning
    reasoning = selected.get("proposal_reasoning", "")
    if reasoning:
    ...
    approved = st.checkbox("Approve proposal", value=True, key=f"approve_{tab_key}")
    reason = st.text_input(
        "Reason",
        value="approved by user" if approved else "rejected by user",
        key=f"reason_{tab_key}",
    )

    return selected_idx, approved, reason
...
tab_dashboard, tab_explorer, tab_memory, tab_tech = st.tabs([
    "Dashboard",
    "Artifact Explorer",
    "Memory",
    "Technical",
...
with tab_dashboard:
    ...
    with col_a1:
        if st.button("Run Assessment", type="primary",
                      help="Runs assessment + generates proposal. Halts at approval for human review."):
    ...
    if st.session_state.get("proposal_data"):
        ...
        st.caption("Review the proposal below. Select a proposal, then approve or reject to continue.")

        proposal = st.session_state.proposal_data
        app_id = proposal.get("app_id", "")

        review_result = _render_proposal_review(proposal, "dashboard")
        if review_result:
            selected_idx, approved, reason = review_result

            if st.button("Execute Decision", type="primary", key="execute_dashboard"):
    ...
    if st.session_state.get("execution_result"):
        ...
        _render_result_summary(st.session_state.execution_result)

        if st.button("Clear Result", key="clear_result_dashboard"):
            st.session_state.execution_result = None
            st.rerun()
...
with tab_memory:
        ...
        with st.spinner("Running A/B test..."):
            try:
                ...
                with col1:
                    ...
                    _render_result_summary(ab_result.get("with_memory", {}), "ab_with")
                with col2:
                    st.markdown("**Without Memory**")
                    _render_result_summary(ab_result.get("without_memory", {}), "ab_without")
    ...
    try:
        ...
        _render_result_summary(metrics, "ema_metrics")
    except Exception as e:
    ...
    if outcomes:
        for o in outcomes:
            ...
            with st.expander(label):
                _render_result_summary(o, f"outcome_{o.get('outcome_id', '')}")
    else:
...
with tab_tech:
    ...
    if st.button("Execute Query", key="mcp_exec"):
        with st.spinner("Querying CockroachDB..."):
            ...
            _render_result_summary(result, "mcp_result")
                ...
                for r in results:
                    with st.expander(f"{r.name} — Severity: {r.severity}"):
                        ...
                        if hasattr(r, "to_dict"):
                            _render_result_summary(r.to_dict(), f"crosscheck_{r.name}")
            else:
    ...
    if st.button("Trigger S3 Assessment", key="s3_trigger"):
        with st.status("Running AWS event chain test...", expanded=True) as status:
            try:
                ...
                _render_result_summary(result, "aws_event")
            except Exception as e:
